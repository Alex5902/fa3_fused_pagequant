...
#include "kernels/fa3_fused_dequant.cuh"
...
template <int Stages, class ClusterShape_, class TileShape_MNK_, int kHeadDimV, class Element_, class ElementAccum_, class ArchTag_,
        ...
        bool MmaPV_is_RS, bool IntraWGOverlap, bool PackGQA_, bool Split_, bool V_colmajor_, bool Has_QKV_>
struct CollectiveMainloopFwdSm90 {
    ...
    static constexpr bool Has_QKV = Has_QKV_;
    ...
    // Commit a producer stage for both pipeline flavors (TMA vs cp.async)
    CUTLASS_DEVICE static
    void commit_stage_auto(MainloopPipelineK &pipe, PipelineState const& st, uint32_t bytes) { // <-- Add bytes argument
        cutlass::arch::fence_view_async_shared();
        if constexpr (Use_TMA_KV) {
            pipe.producer_commit(st, bytes); // <-- Pass bytes to the TMA commit
        } else {
            pipe.producer_commit(st, cutlass::arch::cpasync_barrier_arrive);
        }
    }

    CUTLASS_DEVICE static
    void commit_stage_auto_v(MainloopPipelineV &pipe, PipelineState const& st, uint32_t bytes) {
        cutlass::arch::fence_view_async_shared();
        if constexpr (Use_TMA_KV && !Transpose_V) {
            pipe.producer_commit(st, bytes);
        } else {
            pipe.producer_commit(st, cutlass::arch::cpasync_barrier_arrive);
        }
    }
    ...
    struct Arguments {
        ...
        // ====== new quantized-KV args ======
        const int8_t* const ptr_Kq = nullptr;           // int8 K: (seqlen, d, head, batch)
        const int8_t* const ptr_Vq = nullptr;           // int8 V: (seqlen, d_v, head, batch)

        const __half* const scales_K = nullptr;         // per-(frag_N, frag_K) scales for K
        const __half* const scales_V = nullptr;         // per-(frag_N, frag_K) scales for V
        const int8_t* const zpoints_K = nullptr;        // optional zero-points (or nullptr)
        const int8_t* const zpoints_V = nullptr;        // optional zero-points (or nullptr)

        // Tile the scales with these micro-frag sizes (match your tested path)
        int const frag_m = 16;                          // rows (N direction) per micro-frag
        int const frag_k = 64;                          // cols (head_dim) per micro-frag

        // +++ START OF CHANGES +++
        using ShapeQKV = cute::Shape<int32_t, int32_t, int32_t, int32_t>;
        using StrideQK = cute::Stride<int64_t, _1, int64_t, int64_t>;
        using StrideV = std::conditional_t<!V_colmajor, StrideQK, cute::Stride<_1, int64_t, int64_t, int64_t>>;
        using ShapeScales = cute::Shape<int32_t, int32_t, int32_t, int32_t>;
        using LayoutScales = decltype(cute::make_layout(ShapeScales{}, cute::GenRowMajor{}));

        ShapeQKV const shape_Kq;
        StrideQK const stride_Kq;
        ShapeQKV const shape_Vq;
        StrideV  const stride_Vq;

        LayoutScales const layout_K_scales;
        LayoutScales const layout_V_scales;
        LayoutScales const layout_K_zps;
        LayoutScales const layout_V_zps;
    };
    ...
    struct Params {
        ...
        // ====== quantized-KV params ======
        const int8_t* const ptr_Kq;
        const int8_t* const ptr_Vq;
        ShapeQKV  const shape_Kq;
        StrideQK  const stride_Kq;
        ShapeQKV  const shape_Vq;
        StrideV   const stride_Vq;
        const __half* const scales_K;
        const __half* const scales_V;
        const int8_t* const zpoints_K;
        const int8_t* const zpoints_V;
        int const frag_m;
        int const frag_k;
        // ====== quantized-KV params ======
        using ShapeScales = cute::Shape<int32_t, int32_t, int32_t, int32_t>;
        using LayoutScales = decltype(cute::make_layout(ShapeScales{}, cute::GenRowMajor{}));
        LayoutScales const layout_K_scales, layout_V_scales;
        LayoutScales const layout_K_zps, layout_V_zps;
    };
    ...
    to_underlying_arguments(Arguments const& args) {
        ...
        // keep the divisor as constructed; if you must reflect the arg, clamp it:
        // if (args.attention_chunk >= 1) attention_chunk_divmod.divisor = args.attention_chunk;
        ...
        return {args.ptr_Q, args.shape_Q, args.stride_Q, shape_Q_packed, stride_Q_packed,
                ...
                args.ptr_Kq, args.ptr_Vq,
                args.shape_Kq, args.stride_Kq,
                args.shape_Vq, args.stride_Vq,
                args.scales_K, args.scales_V,
                args.zpoints_K, args.zpoints_V,
                args.frag_m, args.frag_k,
                args.layout_K_scales, args.layout_V_scales,
                args.layout_K_zps, args.layout_V_zps
            };
    }
    ...
    // ====== helper block: fused dequant into FA-3 SMEM (no transpose path) ======
    CUTLASS_DEVICE
    void dequant_block_q8_to_smem_no_transpose_K(
        const int8_t* __restrict__ Kq,
        ShapeQKV const& shape_Kq,
        StrideQK const& stride_Kq,
        const __half* __restrict__ scales,
        const int8_t* __restrict__ zps,
        // stage-sliced, PI-swizzled SMEM tile over (N, K) — no stage coord here
        auto const& sK_pi,
        // tile origin + valid extents
        int N0, int N_valid, int K_total,
        int frag_m, int frag_k,
        int thread_idx // producer-local thread index
    ) {
        using Elem = Element;
        if (thread_idx >= NumProducerThreads) return; // only producer threads
        int lane = thread_idx & 31;

        // grid of micro-tiles covering this (kBlockN × head_dim) tile
        const int mF_total = (get<0>(shape_Kq) + frag_m - 1) / frag_m;   // over full seqlen
        const int kF_total = (K_total         + frag_k - 1) / frag_k;

        // row-chunks within this block tile
        for (int rr = 0; rr < N_valid; rr += frag_m) {
            int m_chunk = min(frag_m, N_valid - rr);

            // columns across head_dim in frag_k steps
            for (int kk = 0; kk < K_total; kk += frag_k) {
                int k_chunk = min(frag_k, K_total - kk);

                // per-(frag_m, frag_k) scale / zp
                int rowFrag = (N0 + rr) / frag_m;
                int colFrag = kk / frag_k;
                int sk_idx  = rowFrag * kF_total + colFrag;
                float s     = scales ? __half2float(scales[sk_idx]) : 1.f;
                float zp    = zps ? float(zps[sk_idx]) : 0.f;

                int warp = thread_idx >> 5;
                int num_warps = NumProducerThreads / 32;

                int passes = (frag_m + 31) / 32;
                // each lane handles one row within frag_m at a time
                for (int pass = warp; pass < passes; pass += num_warps) {
                    int r = pass * 32 + lane;
                    if (r >= m_chunk) break;

                    // base ptr of this (absolute) row in global Kq: (seqlen, d, head, batch)
                    int n_abs = N0 + rr + r;
                    int n_loc = rr + r;
                    // NOTE: strides are in *elements* of int8
                    int64_t g_off =
                        int64_t(n_abs)                   * get<0>(stride_Kq) +
                        int64_t(0)                       * get<1>(stride_Kq) +   // d will be added below
                        int64_t(0)                       * get<2>(stride_Kq) +   // head handled outside this file (we're in 1 head here)
                        int64_t(0)                       * get<3>(stride_Kq);    // batch idem
                    const int8_t* __restrict__ g_row = Kq + g_off;

                    // unpack 16 int8 → 16 half and store into SMEM in the FA-3 layout
                    // iterate columns of this micro-tile in 16-elem vectors
                    for (int c = 0; c < k_chunk; c += 16) {
                        int v_elems = min(16, k_chunk - c);

                        // tmp buffer
                        Elem h16[16];

                        // vector path if aligned & full 16
                        const int8_t* src = g_row + (kk + c) * get<1>(stride_Kq);
                        bool contiguous_d = (get<1>(stride_Kq) == 1);
                        if (v_elems == 16 && contiguous_d && (((uintptr_t)src & 0xF) == 0)) {
                            int4 v = *reinterpret_cast<const int4*>(src);
                            fa3_dequant_pack16_to_elem<Elem>(v, s, zp, h16);
                        } else {
                            // scalar tail
                            cutlass::NumericConverterClamp<Elem, float> to_elem;
                            #pragma unroll
                            for (int t = 0; t < 16; ++t) {
                                float q = (t < v_elems) ? float(src[t * get<1>(stride_Kq)]) : 0.f;
                                h16[t] = to_elem((q - zp) * s);
                            }
                        }

                        // store 16 half into FA-3 SMEM at (row=n_abs, col=kk+c..kk+c+15, stage)
                        // use the position-independent swizzle tensor to compute address
                        fa3_smem_store16_interleaved_tile<Elem, kBlockN, kHeadDim, 16>(
                            sK_pi.data().get(), n_loc, kk + c, h16, /*row_is_v=*/false);
                    } // columns in micro-tile
                } // pass over lanes
            } // kk
        } // rr
    }
    ...
    CUTLASS_DEVICE
    void dequant_block_q8_to_smem_no_transpose_V(
        const int8_t* __restrict__ Vq,
        ShapeQKV const& shape_Vq,
        StrideV  const& stride_Vq,
        const __half* __restrict__ scales,
        const int8_t* __restrict__ zps,
        auto const& sV_pi,   // PI-swizzled, stage-sliced, over (N, Kv)
        int N0, int N_valid, int Kv_total,
        int frag_m, int frag_k,
        int thread_idx
    ) {
        using Elem = Element;
        ...
        const int mF_total = (get<0>(shape_Vq) + frag_m - 1) / frag_m;
        const int kF_total = (Kv_total         + frag_k - 1) / frag_k;

        for (int rr = 0; rr < N_valid; rr += frag_m) {
            int m_chunk = min(frag_m, N_valid - rr);
            for (int kk = 0; kk < Kv_total; kk += frag_k) {
                int k_chunk = min(frag_k, Kv_total - kk);
                int rowFrag = (N0 + rr) / frag_m;
                int colFrag = kk / frag_k;
                int sk_idx  = rowFrag * kF_total + colFrag;
                float s     = scales ? __half2float(scales[sk_idx]) : 1.f;
                ...
                for (int pass = warp; pass < passes; pass += num_warps) {
                    ...
                    int n_loc = rr + r;
                    int64_t g_off =
                        int64_t(n_abs)                   * get<0>(stride_Vq) +
                        int64_t(0)                       * get<1>(stride_Vq) +
                        int64_t(0)                       * get<2>(stride_Vq) +
                        int64_t(0)                       * get<3>(stride_Vq);
                    const int8_t* __restrict__ g_row = Vq + g_off;

                    for (int c = 0; c < k_chunk; c += 16) {
                        int v_elems = min(16, k_chunk - c);
                        Elem h16[16];

                        const int8_t* src = g_row + (kk + c) * get<1>(stride_Vq);
                        bool contiguous_d = (get<1>(stride_Vq) == 1);
                        if (v_elems == 16 && contiguous_d && (((uintptr_t)src & 0xF) == 0)) {
                            int4 v = *reinterpret_cast<const int4*>(src);
                            fa3_dequant_pack16_to_elem<Elem>(v, s, zp, h16);
                        } else {
                            ...
                            #pragma unroll
                            for (int t = 0; t < 16; ++t) {
                                float q = (t < v_elems) ? float(src[t * get<1>(stride_Vq)]) : 0.f;
                                h16[t] = to_elem((q - zp) * s);
                            }
                        }

                        fa3_smem_store16_interleaved_tile<Elem, kBlockN, kHeadDimV, 16>(
                            sV_pi.data().get(),
                            /*r=*/n_loc,
                            /*c_start=*/kk + c,
                            h16,
    ...
    load(Params const& params,
        ...
        if (should_print) {
            printf("--- KERNEL: Entered load method. m_block=%d, bidh=%d, bidb=%d, n_block_min=%d, n_block_max=%d\n",
        ...
        // q8 fused path is only supported on the no-transpose V path here
        // Only support non-FP8, no transpose, and V not colmajor for the q8 fused path.
        // static constexpr bool UseQ8Here = (!Is_FP8 && !Transpose_V && !V_colmajor && Use_TMA_KV && !AppendKV);
        // bool use_q8_k = UseQ8Here && params.ptr_Kq && params.scales_K;
        // bool use_q8_v = UseQ8Here && params.ptr_Vq && params.scales_V;
        ...
        auto load_K = [&] (int const n_block, auto const& smem_pipe_write, auto need_seqlenk_masking_type) {
            // Use a compile-time branch to select the kernel's behavior
            if constexpr (Has_QKV) {
                if (should_print) {
                    printf("--- KERNEL (load_K): Taking INT8 dequant path for K, n_block=%d\n", n_block);
                }
                // =============================================================
                // PATH 1: Fused INT8 Dequantization (this is YOUR new code)
                // This code only gets compiled when Has_QKV is true.
                // =============================================================
                pipeline_k.producer_acquire(smem_pipe_write);
                constexpr int kBlockN = get<1>(TileShape_MNK{});
                int N0 = seqlen_info.offset_k + n_block * kBlockN;
                constexpr bool Seqlenk_mask = decltype(need_seqlenk_masking_type)::value;
                int N_valid = Seqlenk_mask ? max(0, min(kBlockN, seqlen_info.seqlen_k - n_block * kBlockN)) : kBlockN;

                const __half* scalesK = params.scales_K;
                const int8_t* zpsK    = params.zpoints_K;
                // Pre-offset scales/zps by (head,batch) if they exist
                // if (scalesK || zpsK) {
                //     int mF_total = (get<0>(params.shape_Kq) + params.frag_m - 1) / params.frag_m;
                //     int kF_total = (get<2>(TileShape_MNK{}) + params.frag_k - 1) / params.frag_k;
                //     int sk_stride = mF_total * kF_total;
                //     int const bidb_kv_loc = params.kv_batch_idx == nullptr ? bidb : params.kv_batch_idx[bidb];
                //     int const head_batch  = bidh_kv + get<2>(params.shape_Kq) * bidb_kv_loc;
                //     if (scalesK) scalesK += head_batch * sk_stride;
                //     if (zpsK)    zpsK    += head_batch * sk_stride;
                // }
                if (scalesK) {
                    int const bidb_kv_loc = params.kv_batch_idx == nullptr ? bidb : params.kv_batch_idx[bidb];
                    scalesK += params.layout_K_scales(make_coord(bidb_kv_loc, bidh_kv, 0, 0));
                }
                if (zpsK) {
                    int const bidb_kv_loc = params.kv_batch_idx == nullptr ? bidb : params.kv_batch_idx[bidb];
                    zpsK += params.layout_K_zps(make_coord(bidb_kv_loc, bidh_kv, 0, 0));
                }

                int const bidb_kv_loc = params.kv_batch_idx == nullptr ? bidb : params.kv_batch_idx[bidb];
                const int8_t* KqHB = params.ptr_Kq
                                + bidh_kv     * get<2>(params.stride_Kq)
                                + bidb_kv_loc * get<3>(params.stride_Kq);

                auto sK_pi_stage = sK_pi(_, _, smem_pipe_write.index());

                int thread_idx_prod = threadIdx.x % NumProducerThreads;
                dequant_block_q8_to_smem_no_transpose_K(
                    KqHB, params.shape_Kq, params.stride_Kq,
                    scalesK, zpsK, sK_pi_stage,
                    N0, N_valid, get<2>(TileShape_MNK{}),
                    params.frag_m, params.frag_k, thread_idx_prod
                );

                if (should_print) {
                    printf("--- KERNEL (load_K): Dequant for K finished, n_block=%d. Committing stage.\n", n_block);
                }
                // cutlass::arch::fence_view_async_shared();
                // pipeline_k.producer_commit(smem_pipe_write, cutlass::arch::cpasync_barrier_arrive);
                commit_stage_auto(pipeline_k, smem_pipe_write, TmaTransactionBytesK);
                // Note: No "return;" here because the if constexpr is a complete branch.
            } else {
                // =============================================================
                // PATH 2: Original FP16 Logic
                // This code only gets compiled when Has_QKV is false.
                // =============================================================
                pipeline_k.producer_acquire(smem_pipe_write);
            ...
            // The duplicated code that was here has been removed.
        };

        auto load_V = [&] (int const n_block, auto const& smem_pipe_write, auto need_seqlenk_masking_type) {
            if constexpr (Has_QKV) {
                if (should_print) {
                    printf("--- KERNEL (load_V): Taking INT8 dequant path for V, n_block=%d\n", n_block);
                }
                // =============================================================
                // PATH 1: Fused INT8 Dequantization for V
                // =============================================================
                auto pipeline_v_load = pipeline_v; // no-transpose path
                ...
                const __half* scalesV = params.scales_V;
                const int8_t* zpsV    = params.zpoints_V;
                // if (scalesV || zpsV) {
                //     int mF_total = (get<0>(params.shape_Vq) + params.frag_m - 1) / params.frag_m;
                //     int kF_total = (int(params.headdim_v) + params.frag_k - 1) / params.frag_k;
                //     int sk_stride = mF_total * kF_total;
                //     int const bidb_kv_loc = params.kv_batch_idx == nullptr ? bidb : params.kv_batch_idx[bidb];
                //     int const head_batch  = bidh_kv + get<2>(params.shape_Vq) * bidb_kv_loc;
                //     if (scalesV) scalesV += head_batch * sk_stride;
                //     if (zpsV)    zpsV    += head_batch * sk_stride;
                // }
                if (scalesV) {
                    int const bidb_kv_loc = params.kv_batch_idx == nullptr ? bidb : params.kv_batch_idx[bidb];
                    scalesV += params.layout_V_scales(make_coord(bidb_kv_loc, bidh_kv, 0, 0));
                }
                if (zpsV) {
                    int const bidb_kv_loc = params.kv_batch_idx == nullptr ? bidb : params.kv_batch_idx[bidb];
                    zpsV += params.layout_V_zps(make_coord(bidb_kv_loc, bidh_kv, 0, 0));
                }

                int const bidb_kv_loc = params.kv_batch_idx == nullptr ? bidb : params.kv_batch_idx[bidb];
                const int8_t* VqHB = params.ptr_Vq
                   + bidh_kv     * get<2>(params.stride_Vq)
                   + bidb_kv_loc * get<3>(params.stride_Vq);

                auto sVt_pi_stage = cute::as_position_independent_swizzle_tensor(sVt)(_, _, smem_pipe_write.index());

                int thread_idx_prod = threadIdx.x % NumProducerThreads;
                dequant_block_q8_to_smem_no_transpose_V(
                    VqHB, params.shape_Vq, params.stride_Vq,
                    scalesV, zpsV, sVt_pi_stage,
                    N0, N_valid, int(params.headdim_v),
                    params.frag_m, params.frag_k, thread_idx_prod
                );

                if (should_print) {
                    printf("--- KERNEL (load_V): Dequant for V finished, n_block=%d. Committing stage.\n", n_block);
                }
                // cutlass::arch::fence_view_async_shared();
                // pipeline_v_load.producer_commit(smem_pipe_write, cutlass::arch::cpasync_barrier_arrive);
                commit_stage_auto_v(pipeline_v_load, smem_pipe_write, TmaTransactionBytesV);
            } else {
                // =============================================================
                // PATH 2: Corrected Original FP16 Logic for V
                // =============================================================
        ...
        bool elected =
            ((SingleProducerWarp || warp_idx_in_warpgroup == 0) && cute::elect_one_sync());

        // One election for page table (shared by K and/or V)
        // bool should_load_any =
        //     (!Use_TMA_KV || use_q8_k || use_q8_v) || elected;

        // // Separate elections for K and V (so only one thread does each TMA when not q8)
        // bool should_load_K =
        //     (!Use_TMA_KV || use_q8_k) || elected;

        // bool should_load_V =
        //     (!Use_TMA_KV || use_q8_v) || elected;

        // The QKV path is fundamentally a non-TMA path for K and V.
        static constexpr bool IsTMA_KV = Use_TMA_KV && !Has_QKV;

        bool should_load_any = !IsTMA_KV || elected;
        bool should_load_K = !IsTMA_KV || elected;
        bool should_load_V = !IsTMA_KV || elected;
        ...
        if (should_load_any) {
            ...
            if constexpr (Transpose_V) { if (should_load_V) load_V(n_block, smem_pipe_write, cute::true_type{} /*Seqlenk_mask*/); }
        ...
        for (; n_block >= n_block_min; --n_block) {
            if (should_print) {
                printf("--- KERNEL (load): Main loop iteration, n_block = %d\n", n_block);
            }
            ...
            if (should_load_any) {
                ...
                if constexpr (Transpose_V) { if (should_load_V) load_V(n_block, smem_pipe_write, cute::false_type{} /*Seqlenk_mask*/); }
        ...
        if (should_print) {
             printf("--- KERNEL (load): Exited main loop.\n");
        }
        scheduler_prefetch();
        if constexpr (!Transpose_V && IntraWGOverlap) {
    ...
    mma(Params const& params,
        ...
        if (should_print) {
            printf("--- KERNEL: Entered mma method. m_block=%d, n_block_min=%d, n_block_max=%d\n",
        ...
        if constexpr (IntraWGOverlap) {
            ...
            for (; n_block >= n_block_min_before_local_mask; --n_block) {
                if (should_print) {
                    printf("--- KERNEL (mma): Exited main compute loop.\n");
                }
