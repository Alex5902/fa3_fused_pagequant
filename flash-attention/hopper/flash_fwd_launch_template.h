...
template <int Arch, int kHeadDim, int kHeadDimV, int ClusterM, typename Element, typename ElementOut,
          ...
          bool PackGQA, bool Split, bool V_colmajor, bool Has_QKV>
void run_flash_fwd(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    using CollectiveMainloop = std::conditional_t<
        Arch >= 90,
        flash::CollectiveMainloopFwdSm90<kStages, ClusterShape, TileShape_MNK, kHeadDimV, Element, float, cutlass::arch::Sm90, Is_causal, Is_local, Has_softcap, Varlen, PagedKVNonTMA, AppendKV, HasQv, MmaPV_is_RS, IntraWGOverlap, PackGQA, Split, V_colmajor, Has_QKV>,
    ...
    // ==================== MODIFICATION 1: RECONSTRUCT CUTE OBJECTS (Host-side) ====================
    // These definitions are for the shapes/strides of your NEW tensors.
    using ShapeQKV = cute::Shape<int32_t, int32_t, int32_t, int32_t>;
    using StrideQK = cute::Stride<int64_t, cute::_1, int64_t, int64_t>;
    ...
    using StrideScales = cute::Stride<int64_t, int64_t, int64_t, int64_t>;
    
    // Create the shape/stride objects from the raw integer params
    auto shape_Kq = cute::make_shape(params.seqlen_k, params.d, params.h_k, params.b);
    auto stride_Kq = cute::make_stride(params.kq_row_stride, cute::_1{}, params.kq_head_stride, params.kq_batch_stride);
    auto shape_Vq = cute::make_shape(params.seqlen_k, params.dv, params.h_k, params.b);
    auto stride_Vq = cute::make_stride(params.vq_row_stride, cute::_1{}, params.vq_head_stride, params.vq_batch_stride);
    
    int seqlen_for_scales = !params.cu_seqlens_q ? params.seqlen_q : params.total_q;
    int num_seq_frags = (seqlen_for_scales + params.frag_m - 1) / params.frag_m;
    int num_dim_frags = (params.d + params.frag_k - 1) / params.frag_k;
    int num_dim_v_frags = (params.dv + params.frag_k - 1) / params.frag_k;

    auto shape_K_scales = cute::make_shape(params.b, params.h_k, num_seq_frags, num_dim_frags);
    // row-major strides for (B, H, Sfrag, Dfrag) with last stride compile-time 1
    int sK_B = params.h_k * num_seq_frags * num_dim_frags;
    int sK_H =               num_seq_frags * num_dim_frags;
    int sK_S =                              num_dim_frags;
    auto layout_K_scales = cute::make_layout(
        shape_K_scales,
        cute::make_stride(sK_B, sK_H, sK_S, cute::Int<1>{})
    );

    auto shape_V_scales = cute::make_shape(params.b, params.h_k, num_seq_frags, num_dim_v_frags);
    int sV_B = params.h_k * num_seq_frags * num_dim_v_frags;
    int sV_H =               num_seq_frags * num_dim_v_frags;
    int sV_S =                              num_dim_v_frags;
    auto layout_V_scales = cute::make_layout(
        shape_V_scales,
        cute::make_stride(sV_B, sV_H, sV_S, cute::Int<1>{})
    );
    
    auto layout_K_zps = layout_K_scales;
    auto layout_V_zps = layout_V_scales;
    // =============================================================================================
    ...
    typename CollectiveMainloop::Arguments mainloop_args {
        ...
        static_cast<const int8_t*>(params.kq_ptr),
        static_cast<const int8_t*>(params.vq_ptr),
        static_cast<const __half*>(params.k_scales_ptr),
        static_cast<const __half*>(params.v_scales_ptr),
        static_cast<const int8_t*>(params.k_zps_ptr),
        static_cast<const int8_t*>(params.v_zps_ptr),
        params.frag_m,
        params.frag_k,
        shape_Kq,
        stride_Kq,
...
template<int Arch, typename T, int kHeadDim, int kHeadDimV, bool Split, bool PagedKVNonTMA, bool Has_softcap, bool PackGQA, bool Has_QKV>
                ...
                BOOL_SWITCH(params.qv_ptr, HasQV_, [&] {
                    ...
                    APPENDKV_SWITCH(params.knew_ptr, AppendKV, [&] {
                        CLUSTER_SWITCH(cutlass::ceil_div(params.seqlen_q * (!PackGQA ? 1 : params.h / params.h_k), kBlockM) % 2 == 0, Use_cluster, [&] {
                            ...
                            // Pass the new `Has_QKV` flag to run_flash_fwd
                            run_flash_fwd<Arch, kHeadDim, kHeadDimV, ClusterM, T, T_out, Is_causal, Is_local, Has_softcap, Varlen, PagedKVNonTMA, AppendKV && Varlen, HasQv, PackGQA, Split, V_colmajor, Has_QKV>(params, stream);
                        });
                    });
                });
            });
