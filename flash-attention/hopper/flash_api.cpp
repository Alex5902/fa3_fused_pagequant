...
// If we're building forward-only, force-disable all backward code paths.
#if defined(FA3_BUILD_FWD_ONLY) && !defined(FLASHATTENTION_DISABLE_BACKWARD)
#define FLASHATTENTION_DISABLE_BACKWARD 1
#endif
...
#define FA3_EXPORT __attribute__((visibility("default")))
#define FA3_USED   __attribute__((used))
#else
#define FA3_EXPORT
#define FA3_USED
#endif
...
/* Creates a dummy empty _C module that can be imported from Python.
   Importing runs the TORCH_LIBRARY static initializers below. */
FA3_EXPORT FA3_USED PyObject* PyInit__C(void)
{
...
/* Also export PyInit_fa3_cuda so `import fa3_cuda` works too */
FA3_EXPORT FA3_USED PyObject* PyInit_fa3_cuda(void) {
    return PyInit__C();
}
...
void set_params_fprop(Flash_fwd_params &params,
                      ...
                      const at::Tensor k_q = {},
                      const at::Tensor v_q = {},
                      const at::Tensor k_scales = {},
                      const at::Tensor v_scales = {},
                      const at::Tensor k_zps = {},
                      const at::Tensor v_zps = {},
                      const int frag_m = 0,
                      const int frag_k = 0) {
    ...
    if (k_q.defined()) {
        params.kq_ptr = k_q.data_ptr<int8_t>();
        params.kq_row_stride = k_q.stride(-3);
        params.kq_head_stride = k_q.stride(-2);
        if (params.cu_seqlens_k == nullptr) { params.kq_batch_stride = k_q.stride(0); }
    }
    if (v_q.defined()) {
        params.vq_ptr = v_q.data_ptr<int8_t>();
        params.vq_row_stride = v_q.stride(-3);
        params.vq_head_stride = v_q.stride(-2);
        if (params.cu_seqlens_k == nullptr) { params.vq_batch_stride = v_q.stride(0); }
    }
    if (k_scales.defined()) {
        params.k_scales_ptr = k_scales.data_ptr<at::Half>(); // Or __half if you pass float16
    }
    if (v_scales.defined()) {
        params.v_scales_ptr = v_scales.data_ptr<at::Half>(); // Or __half if you pass float16
    }
    if (k_zps.defined()) { params.k_zps_ptr = k_zps.data_ptr<int8_t>(); }
    if (v_zps.defined()) { params.v_zps_ptr = v_zps.data_ptr<int8_t>(); }
    params.frag_m = frag_m;
    params.frag_k = frag_k;
...
template <int Arch, int Split, bool PagedKVNonTMA, bool PackGQA, bool Has_softcap, bool Has_QKV>
void run_mha_fwd_constexpr(Flash_fwd_params &params, cudaStream_t stream) {
    if (!params.is_e4m3) {
        if (params.is_bf16) {
            ...
            if (params.d <= 64) {
                ...
                if constexpr (Arch == 90) {
                    if (params.dv > 256) {
                        return run_mha_fwd_<Arch, cutlass::bfloat16_t, 64, 512, Split, PagedKVNonTMA, Has_softcap, PackGQA, Has_QKV>(params, stream);
                    } else if (params.dv > 64) {
                        return run_mha_fwd_<Arch, cutlass::bfloat16_t, 64, 256, Split, PagedKVNonTMA, Has_softcap, PackGQA, Has_QKV>(params, stream);
                    }
                }
                #endif
                return run_mha_fwd_<Arch, cutlass::bfloat16_t, 64, 64, Split, PagedKVNonTMA, Has_softcap, PackGQA, Has_QKV>(params, stream);
            }
            #endif
            #ifndef FLASHATTENTION_DISABLE_HDIM96
            if (params.d <= 96) { return run_mha_fwd_<Arch, cutlass::bfloat16_t, 96, 96, Split, PagedKVNonTMA, Has_softcap, PackGQA, Has_QKV>(params, stream); }
            #endif
            #ifndef FLASHATTENTION_DISABLE_HDIM128
            if (params.d <= 128) { return run_mha_fwd_<Arch, cutlass::bfloat16_t, 128, 128, Split, PagedKVNonTMA, Has_softcap, PackGQA, Has_QKV>(params, stream); }
            #endif
            #ifndef FLASHATTENTION_DISABLE_HDIM192
            if (params.d <= 192) {
                #ifndef FLASHATTENTION_DISABLE_HDIMDIFF192
                if constexpr (Arch == 90) {
                    if (params.dv <= 128) {
                        return run_mha_fwd_<Arch, cutlass::bfloat16_t, 192, 128, Split, PagedKVNonTMA, Has_softcap, PackGQA, Has_QKV>(params, stream);
                    }
                }
                #endif
                return run_mha_fwd_<Arch, cutlass::bfloat16_t, 192, 192, Split, PagedKVNonTMA, Has_softcap, PackGQA, Has_QKV>(params, stream);
            }
            #endif
            #ifndef FLASHATTENTION_DISABLE_HDIM256
            if (params.d <= 256) { return run_mha_fwd_<Arch, cutlass::bfloat16_t, 256, 256, Split, PagedKVNonTMA, Has_softcap, PackGQA, Has_QKV>(params, stream); }
            #endif
        } else {
            ...
            if (params.d <= 64) {
                ...
                if constexpr (Arch == 90) {
                    if (params.dv > 256) {
                        return run_mha_fwd_<Arch, cutlass::half_t, 64, 512, Split, PagedKVNonTMA, Has_softcap, PackGQA, Has_QKV>(params, stream);
                    } else if (params.dv > 64) {
                        return run_mha_fwd_<Arch, cutlass::half_t, 64, 256, Split, PagedKVNonTMA, Has_softcap, PackGQA, Has_QKV>(params, stream);
                    }
                }
                #endif
                return run_mha_fwd_<Arch, cutlass::half_t, 64, 64, Split, PagedKVNonTMA, Has_softcap, PackGQA, Has_QKV>(params, stream);
            }
            #endif
            #ifndef FLASHATTENTION_DISABLE_HDIM96
            if (params.d <= 96) { return run_mha_fwd_<Arch, cutlass::half_t, 96, 96, Split, PagedKVNonTMA, Has_softcap, PackGQA, Has_QKV>(params, stream); }
            #endif
            #ifndef FLASHATTENTION_DISABLE_HDIM128
            if (params.d <= 128) { return run_mha_fwd_<Arch, cutlass::half_t, 128, 128, Split, PagedKVNonTMA, Has_softcap, PackGQA, Has_QKV>(params, stream); }
            #endif
            #ifndef FLASHATTENTION_DISABLE_HDIM192
            if (params.d <= 192) {
                #ifndef FLASHATTENTION_DISABLE_HDIMDIFF192
                if constexpr (Arch == 90) {
                    if (params.dv <= 128) {
                        return run_mha_fwd_<Arch, cutlass::half_t, 192, 128, Split, PagedKVNonTMA, Has_softcap, PackGQA, Has_QKV>(params, stream);
                    }
                }
                #endif
                return run_mha_fwd_<Arch, cutlass::half_t, 192, 192, Split, PagedKVNonTMA, Has_softcap, PackGQA, Has_QKV>(params, stream);
            }
            #endif
            #ifndef FLASHATTENTION_DISABLE_HDIM256
            if (params.d <= 256) { return run_mha_fwd_<Arch, cutlass::half_t, 256, 256, Split, PagedKVNonTMA, Has_softcap, PackGQA, Has_QKV>(params, stream); }
            #endif
            #else
        ...
        #ifndef FLASHATTENTION_DISABLE_FP8
        #ifndef FLASHATTENTION_DISABLE_HDIM64
        if (params.d <= 64) { return run_mha_fwd_<90, cutlass::float_e4m3_t, 64, 64, Split, PagedKVNonTMA, Has_softcap, PackGQA, Has_QKV>(params, stream); }
        #endif
        #ifndef FLASHATTENTION_DISABLE_HDIM96
        if (params.d <= 96) { return run_mha_fwd_<90, cutlass::float_e4m3_t, 96, 96, Split, PagedKVNonTMA, Has_softcap, PackGQA, Has_QKV>(params, stream); }
        #endif
        #ifndef FLASHATTENTION_DISABLE_HDIM128
        if (params.d <= 128) { return run_mha_fwd_<90, cutlass::float_e4m3_t, 128, 128, Split, PagedKVNonTMA, Has_softcap, PackGQA, Has_QKV>(params, stream); }
        #endif
        #ifndef FLASHATTENTION_DISABLE_HDIM192
        if (params.d <= 192) {
            #ifndef FLASHATTENTION_DISABLE_HDIMDIFF192
            if constexpr (Arch == 90) {
                if (params.dv <= 128) {
                    return run_mha_fwd_<90, cutlass::float_e4m3_t, 192, 128, Split, PagedKVNonTMA, Has_softcap, PackGQA, Has_QKV>(params, stream);
                }
            }
            #endif
            return run_mha_fwd_<90, cutlass::float_e4m3_t, 192, 192, Split, PagedKVNonTMA, Has_softcap, PackGQA, Has_QKV>(params, stream);
        }
        #endif
        #ifndef FLASHATTENTION_DISABLE_HDIM256
        if (params.d <= 256) { return run_mha_fwd_<90, cutlass::float_e4m3_t, 256, 256, Split, PagedKVNonTMA, Has_softcap, PackGQA, Has_QKV>(params, stream); }
        #endif
        #else
...
void run_mha_fwd(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    #if defined(FLASHATTENTION_DISABLE_SOFTCAP)
                    // Compile ONLY the non-softcap path
                    constexpr bool Has_softcap = false;
                    QKV_SWITCH(params.kq_ptr != nullptr, Has_QKV, [&] {
                        run_mha_fwd_constexpr<Arch, Split, PagedKVNonTMA, PackGQA, Has_softcap, Has_QKV>(params, stream);
                    });
    #else
...
mha_fwd(at::Tensor q,
        ...
        // New direct arguments instead of dictionary
        std::optional<at::Tensor> k_q_,
        std::optional<at::Tensor> v_q_,
        std::optional<at::Tensor> k_scales_,
        std::optional<at::Tensor> v_scales_,
        std::optional<at::Tensor> k_zps_,
        std::optional<at::Tensor> v_zps_,
        std::optional<int64_t> frag_m_,
        std::optional<int64_t> frag_k_,
    ...
    std::cout << "--- C++ API: Entered mha_fwd function ---" << std::endl;
    if (k_q_.has_value()) {
        std::cout << "--- C++ API: SUCCESS! k_q_ tensor HAS a value." << std::endl;
        auto k_q_tensor = k_q_.value();
        std::cout << "--- C++ API: k_q tensor info: shape=" << k_q_tensor.sizes()
                  << ", dtype=" << k_q_tensor.scalar_type() << std::endl;
    } else {
        std::cout << "--- C++ API: FAILURE! k_q_ tensor is EMPTY (std::nullopt)." << std::endl;
    }
    ...
    at::Tensor k_q = k_q_.has_value() ? k_q_.value() : at::Tensor();
    at::Tensor v_q = v_q_.has_value() ? v_q_.value() : at::Tensor();
    at::Tensor k_scales = k_scales_.has_value() ? k_scales_.value() : at::Tensor();
    at::Tensor v_scales = v_scales_.has_value() ? v_scales_.value() : at::Tensor();
    at::Tensor k_zps = k_zps_.has_value() ? k_zps_.value() : at::Tensor();
    at::Tensor v_zps = v_zps_.has_value() ? v_zps_.value() : at::Tensor();
    int frag_m = frag_m_.has_value() ? frag_m_.value() : 0;
    int frag_k = frag_k_.has_value() ? frag_k_.value() : 0;
    ...
    set_params_fprop(params,
                     ...
                     q, // Always pass original q
                     k, // Always pass original k
                     v, // Always pass original v
                     out,
                     ...
                     softcap,
                     sm_margin,
                     // Pass the new parsed tensors at the end
                     k_q, v_q, k_scales, v_scales, k_zps, v_zps, frag_m, frag_k);
    ...
    if (total_q > 0 && (total_k + params.total_knew) > 0 && num_heads_k > 0) {
        ...
        std::cout << "--- C++ API: About to call run_mha_fwd. "
                  << "head_dim=" << params.d
                  << ", is_int8_kv=" << (params.kq_ptr != nullptr) // This should be true for your case
                  << " ---" << std::endl;

        run_mha_fwd(params, stream);

        // With CUDA_LAUNCH_BLOCKING=1, this line will ONLY be printed if the kernel
        // finishes successfully. If the kernel hangs, we will never see this message.
        std::cout << "--- C++ API: call to run_mha_fwd has returned ---" << std::endl;
    ...
    std::cout << "--- C++ API: Exiting mha_fwd function ---" << std::endl;
    // return {out, softmax_lse};
    return {out, softmax_lse, out_accum, softmax_lse_accum};
}

#if !defined(FLASHATTENTION_DISABLE_BACKWARD)
...
TORCH_LIBRARY(flash_attn_3, m) {
    ...
    std::cout << "\n\n\n>>>>>>>>> BUILD IS FRESH: flash_api.cpp from 16:16 WAS LOADED <<<<<<<<<\n\n\n" << std::endl;
    ...
    m.def("fwd("
        ...
        // REVISED: Add new optional arguments directly here
        "Tensor? k_q_ = None,"
        "Tensor? v_q_ = None,"
        "Tensor? k_scales_ = None,"
        "Tensor? v_scales_ = None,"
        "Tensor? k_zps_ = None,"
        "Tensor? v_zps_ = None,"
        "int? frag_m_ = None,"
        "int? frag_k_ = None,"
