...
# README: be robust no matter the CWD
...
def _write_ninja_file(path,
    ...
    if with_cuda:
        flags.append(f'cuda_cflags = {" ".join(cuda_cflags)}')
        flags.append(f'cuda_post_cflags = {" ".join(cuda_post_cflags)}')
        # force single-arch everywhere
        cuda_post_cflags_sm80 = cuda_post_cflags
        flags.append(f'cuda_post_cflags_sm80 = {" ".join(cuda_post_cflags_sm80)}')
        cuda_post_cflags_sm80_sm90 = cuda_post_cflags
        flags.append(f'cuda_post_cflags_sm80_sm90 = {" ".join(cuda_post_cflags_sm80_sm90)}')
        cuda_post_cflags_sm100 = cuda_post_cflags
        flags.append(f'cuda_post_cflags_sm100 = {" ".join(cuda_post_cflags_sm100)}')
...
def get_platform():
    ...
    if sys.platform.startswith("linux"):
        ...
        mac_version = ".".join(platform.mac_ver()[0].split(".")[:2]) or "10.9"
...
# NOTE: do not touch git / CUDA at import time; do it only when building from source.

def _make_extensions():
    """Create Extension objects lazily at build time (so bdist_wheel can short-circuit)."""
    # If building sdist/wheel from a clean checkout, make sure submodules are present.
    try:
    ...
    if CUDA_HOME is None:
        raise RuntimeError(
            "CUDA toolkit not found. Set CUDA_HOME (e.g., /usr/local/cuda-12.3) "
            "to build FlashAttention-3 from source."
        )
    ...
    cutlass_dir = repo_dir / "csrc" / "cutlass"  # keep for now (after vendor) or drop if stale
    vendor_root = repo_dir / "third_party" / "nvidia"
    cutlass_vendor = vendor_root / "cutlass" / "include"
    cute_vendor    = vendor_root / "cute"    / "include"
    hopper_kernels_dir = Path(this_dir) / "hopper" / "kernels"
    ...
    if not DISABLE_HDIMDIFF192:
        sources_fwd_sm90 += [f"instantiations/flash_fwd_hdim{hdim}_{dtype}{paged}{split}{softcap}{packgqa}_sm90.cu"
                            ...
                            if not (packgqa and (paged or split))]
    # This generates the list of filenames for your new quantized kernels.
    # It mirrors the logic for sources_fwd_sm90 but adds '_qkv' to the filename.
    sources_fwd_sm90_qkv = [f"instantiations/flash_fwd_hdim{hdim}_{dtype}{paged}{split}{softcap}{packgqa}_qkv_sm90.cu"
                            for hdim, dtype, split, paged, softcap, packgqa in itertools.product(HEAD_DIMENSIONS_FWD, DTYPE_FWD_SM90, SPLIT, PAGEDKV, SOFTCAP, PACKGQA)
                            if not (packgqa and (paged or split))]
    if not DISABLE_HDIMDIFF64:
        sources_fwd_sm90_qkv += [f"instantiations/flash_fwd_hdim{hdim}_{dtype}{paged}{split}{softcap}{packgqa}_qkv_sm90.cu"
    ...
    sources = (
        ["flash_api.cpp"]
        + (sources_fwd_sm80 if not DISABLE_SM8x else []) + sources_fwd_sm90 + sources_fwd_sm90_qkv
    ...
    include_dirs = [
        Path(this_dir),
        hopper_kernels_dir,
        cutlass_vendor,   # <— vendor first
        cute_vendor,      # <— vendor CUTE
        cutlass_dir / "include",  # <— optional: keep AFTER vendor or remove
    ]
    ...
    def _list(name, default_list):
        v = os.getenv(name)
        return default_list if not v else [x.strip() for x in v.split(",") if x.strip()]

    # Defaults if envs aren’t set
    enable_qkv      = _flag("FA3_ENABLE_QKV",      True)
    enable_split    = _flag("FA3_ENABLE_SPLIT",    True)
    enable_softcap  = _flag("FA3_ENABLE_SOFTCAP",  True)
    enable_paged    = _flag("FA3_ENABLE_PAGED",    True)
    enable_packgqa  = _flag("FA3_ENABLE_PACKGQA",  True)
    keep_dtypes     = set(x.lower() for x in _list("FA3_ENABLE_DTYPES", ["fp16","bf16","e4m3"]))
    keep_dims       = set(_list("FA3_HEAD_DIMS",   ["64","96","128","192","256"])) # Removed 512 as it's not in your list

    # For minimal “decode/paged KV” builds it’s handy to drop **all** bwd:
    drop_bwd = _flag("FA3_DROP_BWD", True)

    # Map dtype tokens as they appear in file names (they do match these strings)
    valid_dtype_tokens = {"fp16","bf16","e4m3"}

    def _keep(src):
        b = osp.basename(src)
        # Only touch the generated instantiation units; keep core sources
        if "instantiations" not in src:
            return True
        # 1) drop backward?
        if drop_bwd and b.startswith("flash_bwd_"):
            return False
        # 2) dims
        if not any(f"_hdim{d}_" in b or f"_hdim{d}_" in b.replace("__","_") for d in keep_dims):
            hd = re.findall(r"_hdim(\d+(?:_\d+)?)_", b)
            if not any(part in keep_dims for hit in hd for part in hit.split("_")):
                return False
        # 3) dtype
        dtype_hit = [t for t in valid_dtype_tokens if f"_{t}_" in b]
        if dtype_hit and not any(dt in keep_dtypes for dt in dtype_hit):
            return False
        # 4) features
        if not enable_qkv     and "_qkv_"     in b: return False
        if not enable_split   and "_split_"   in b: return False
        if not enable_softcap and "_softcap_" in b: return False
        if not enable_paged   and "_paged_"   in b: return False
        if not enable_packgqa and "_packgqa_" in b: return False
        return True

    _before = len(sources)
    sources = [s for s in sources if _keep(s)]
    print(f"[fa3/setup] kept {len(sources)}/{_before} source files after env filtering")

    # compute torch lib dir for rpath
    torch_lib_dir = os.path.join(os.path.dirname(torch.__file__), "lib")
    link_args = [
        "-mcmodel=large",
        "-Wl,--gc-sections",
        f"-Wl,-rpath,{torch_lib_dir}",   # optional but convenient
        "-Wl,--as-needed",               # optional: avoid over-linking
    ]
    ...
    return [
        CUDAExtension(
            ...
            extra_compile_args={
                ...
                "nvcc": [
                    "-Xcompiler=-fPIC,-mcmodel=large,-ffunction-sections,-fdata-sections,-fvisibility=hidden",
...
def get_package_version():
    return os.environ.get("FLASH_ATTN_LOCAL_VERSION") or "3.0.0"
...
def get_wheel_url():
    ...
    if torch.version.cuda is None:
        # No CUDA in this torch build -> force source build (or you can add a 'cpu' wheel naming here)
        raise urllib.error.HTTPError(url="", code=404, msg="no CUDA version in torch", hdrs=None, fp=None)
...
class BuildFromSource(BuildExtension):
    """BuildExtension that constructs extensions at run-time so imports stay light-weight."""
    
    # ADD THIS METHOD TO THE CLASS
    def build_extension(self, ext):
        # This is the patch to handle the setuptools incompatibility
        if not hasattr(ext, "_needs_stub"):
            ext._needs_stub = False
        super().build_extension(ext)

    # KEEP YOUR EXISTING run METHOD
    def run(self):
        global ext_modules
...
setup(
   ...
   # Build CUDA extensions only when actually building from source.
   ext_modules=[],
   cmdclass={
       "bdist_wheel": CachedWheelsCommand,
       "build_ext": BuildFromSource,
   },
    python_requires=">=3.8",
    install_requires=[
        "torch",
    ...
    # Do NOT set py_limited_api for PyTorch CUDA extensions.
)
